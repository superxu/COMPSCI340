...
class Process(threading.Thread):
    ...
    def run(self):
        ...
        if self.type == Type.background:
            # for background process, set before run
            self.stoprequest.set()
            self.run_background()
        elif self.type == Type.interactive:
            # for interactive process, set only after input
    ...
    def run_interactive(self):
        ...
        while loops > 0:
            self.dispatcher.waiting_to_runnable(self)
        ...
        # if loops <= 0
        self.dispatcher.remove_process_from_waitinglist(self)
        return
