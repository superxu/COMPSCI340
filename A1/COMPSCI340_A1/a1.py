...
def focus():
    ...
    process.iosys.fill_buffer(process, input)
    process.stoprequest.set()
    return False


def get_runnable_process_from_user(request):
    """Return the process corresponding to the number entered by the user."""
    menu_window.addstr(0, 0, request)
    refresh_menu()
    number = int(menu_window.getstr())
    return the_dispatcher.runnable_process_with_id(number)


def top():
    """Move a runnable process to the top of the stack."""
    process = get_runnable_process_from_user("Enter the number of the process to move:")
    # ...
